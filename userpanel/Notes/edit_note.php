...
$note_id = $_GET['id'];
$selectNote = "SELECT id, title, content from dapf_notes WHERE id=$note_id";
$notes = mysqli_query($conn, $selectNote);
$data = mysqli_fetch_assoc($notes);
if (isset($_POST['updatenote'])) {
    $title = $_POST['title'];
    $content = $_POST['content'];
    $sql = "UPDATE dapf_notes SET title='$title', content='$content' WHERE id=$note_id";
                        ...
                        <div class="">
                            <form class="row gap-2" method="post">
                                ...
                                <div class="col-12">
                                    ...
                                    <input id="title" type="text" name="title" value="<?php echo $data['title'] ?>" required>
                                </div>

                                <div class="col-12 overflow-hidden">
                                    <label for="content" class="cursor-pointer">Content</label>
                                    <textarea id="content" rows="" cols="" name="content"><?php echo $data['content'] ?></textarea>
                                </div>
                                <div class="col-12">
                                    <button type="submit" name="updatenote" class="btn-success">Add Note</button>
                                </div>
                            </form>
                        </div>
                    </div>
