...
$all_incomplete_tasks = $conn->query("SELECT id,date, task_name, task_due_date,user_id ,importance FROM dapf_tasks WHERE `status`!='completed' AND `deleted_status`= 0 AND `user_id`=$user_id");
$no_of_pages = mysqli_num_rows($all_incomplete_tasks);
$start = 0;
$rows_per_page = 10;
...
$selectQuery = "SELECT id,date, task_name, task_due_date,user_id ,importance FROM dapf_tasks WHERE `status`!='completed' AND `deleted_status`= 0 AND `user_id`=$user_id LIMIT $start, $rows_per_page";
